...
with open('output-transpos.txt', 'w') as file:
    file.write(cip)
deciphered_matrix = [[0 for _ in range(len(key))] for _ in range(l)]
idx = 0
for a in lis:
    for k in range(len(key)):
        if a == key[k]:
            for m in range(l):
                deciphered_matrix[m][k] = cip[idx]            
                idx += 1                                     
deciphered_text = ''                                     
for i in range(l):                                     
    for j in range(len(key)):                                                 
        deciphered_text += deciphered_matrix[i][j]                                                     
print('Decrypted text:', deciphered_text)    
with open('decrypted-transpos.txt', 'w') as file:    
